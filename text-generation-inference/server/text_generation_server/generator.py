...
def _mp_fn(rank,
    ...
    def return_to_caller(*data):
        ...
        if rank == 0:
            logger.debug(f"Generator@{rank} returning data")
            xm.mark_step()
            # TODO: data should be a list, to avoid having a multiprocessing value
            mailbox.send(*data)

    while True:
        if rank == 0:
            mailbox.agent_ready.set()
            logger.debug(f"Generator@{rank} waiting for commands")
            mailbox.receive()
        # Wait for rank 0 to receive command
        xm.rendezvous("start")
        logger.debug(f"Generator@{rank} waiting for command at rendezvous")
...
class TpuGenerator(Generator):
    ...
    def info(self) -> InfoResponse:
        s_info = self.mailbox.send(GeneratorCommand.INFO, None)
        return InfoResponse.FromString(s_info)

    def warmup(self, batch: Batch) -> int:
        return self.mailbox.send(GeneratorCommand.WARMUP, batch.SerializeToString())
